...
######################################
# mecab tokenize
mecab -Owakati < $TEST_TRG_RAW > $OUT_DIR/$B.mecab.ref
mecab -Owakati < $OUT_DIR/$B.true.detok > $OUT_DIR/$B.mecab.hyp
cat $OUT_DIR/$B.mecab.hyp | sacrebleu --tokenize=intl $OUT_DIR/$B.mecab.ref | tee -a $OUT_DIR/test.log
